...
var _default = router => {
  ...
  router.get("/cards", _Session.default, async (req, res) => {
    if (!req.user?.cardUserKey) {
      throw new _ApiError.default("User has no credit card", 403, "userHashNoCard");
    }
  ...
  router.delete("/cards/:cardIndex/delete-by-index", _Session.default, async (req, res) => {
    ...
    if (index >= cards?.cardDetails.length) {
      throw new _ApiError.default("Card doesnt exists, checl index number", 400, "ccardIndexInvalid");
    }
    const {
      cardToken
