...
var _default = router => {
  ...
  router.post("/payments/:cartId/with-register-card", _Session.default, async (req, res) => {
    const {
      card
    } = req.body;
    if (!card) {
  ...
  router.post("/payments/:cardId/:cartIndex/with-registered-card-index", _Session.default, async (req, res) => {
    const {
      cartIndex
    } = req.params;
    if (!cartIndex) {
  ...
  router.post("/payments/:cardId/with-registered-card-token", _Session.default, async (req, res) => {
    const {
      cardToken
    } = req.body;
    if (!cardToken) {
      throw new _ApiError.default("Card token is reuired", 400, "cardTokenRequired");
    }
    if (!req.user?.cardUserKey) {
      throw new _ApiError.default("No registered card available", 400, "cardAvailable");
    }
